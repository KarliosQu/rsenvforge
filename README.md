...
在 PowerShell、CMD 或双击 Windows 的 `rsenvforge.exe` 时，不带参数会打开交互启动菜单，可选择轻量、标准、完整安装，以及查看安装记录和系统诊断。选择任一安装等级后，程序会与 `rsenvforge install <等级>` 一样先检测全部工具、显示已安装版本与缺失项，再进入方向键/空格选择缺失组件和 `Y/N` 安装确认。操作结束后会等待按 Enter 关闭窗口。脚本、管道等非交互环境中，无参数只显示帮助，避免阻塞自动化流程。
